...
#' Visualize S-Learner Model Results
#'
#' Generates various diagnostic and summary plots for an S-Learner model object
#' to analyze the distribution and impact of Individual Treatment Effects (ITEs).
#'
#' @param object An object of class `s_learner`, as returned by the corresponding
#'   modeling function.
...
#'           ranked by ITE.}
#'     \item{`"uplift"`}{Uplift curve (or Qini curve) comparing the cumulative
...
#'   be further customized using standard ggplot2 functions.
#'
#' @details
#' This function provides several ways to visualize the results of an S-Learner
#' model:
#' * **Density Plot**: Shows the distribution of Individual Treatment Effects (ITEs).
#'   A dashed vertical line at ITE = 0 is provided for reference. This helps assess
#'   the heterogeneity of treatment effects and the proportion of the population
#'   with positive, negative, or neutral effects.
#' * **Forest Plot**: For `group = NULL`, plots the point estimate of the ITE for
#'   each observation, ranked from highest to lowest. When a `group` is provided,
#'   it displays the mean ITE for each level of the grouping variable, offering a
#'   high-level comparison between subgroups.
#' * **Cumulative Plot**: Also known as the Cumulative Average Effect (CAE) or
#'   "Cumulative Gains" curve. It shows the average ITE one would achieve if
#'   treating the top X% of the population, as ranked by the model's predicted ITE.
#'   A steeper initial curve indicates better prioritization.
#' * **Uplift Plot**: Plots the cumulative incremental gain (uplift) as an
#'   increasingly larger fraction of the population is treated, prioritized by the
#'   predicted ITE. The model's performance is compared against a random targeting
#'   strategy (dashed line). The area between the model's curve and the random line
#'   is the Qini coefficient, a key metric for evaluating uplift models. A higher
#'   coefficient indicates better model performance.
#'
#' @examples
#' \dontrun{
#' # Assuming you have a fitted model named 's_learner_fit'
#' library(ggplot2)
#'
#' # Create an overall ITE density plot
#' autoplot(s_learner_fit, type = "density")
#'
#' # Create a density plot grouped by a variable "age_group"
#' autoplot(s_learner_fit, type = "density", group = "age_group")
#'
#' # Create a forest plot to see all ITE estimates
#' autoplot(s_learner_fit, type = "forest")
#'
#' # Create a cumulative gains curve
#' autoplot(s_learner_fit, type = "cumulative")
#'
#' # Create an uplift curve to evaluate model performance
#' autoplot(s_learner_fit, type = "uplift")
#' }
#' @export
autoplot.s_learner <- function(
    object,
