...
u_learner <- function(
  ...
  # Predict on the counterfactual datasets and original data
  m_hat <- .predict_meta(
  ...
  second_stage <- .second_stage(data = data_resid, m_hat = m_hat, type = "u_learner")
  ...
  cumulative_gain <- .calculate_gain()
  ...
  if (bootstrap) {
    ...
    for (i in seq_len(bootstrap_iters)) {
      ...
      # Build and predict with second stage regression models using data_resid Predict and compute tau
      boot_second_stage <- .second_stage(data = boot_data_resid, m_hat = boot_m_hat, type = "u_learner")

      # Calculate effect measures
      effect_list[[i]] <- .calculate_effects(
        predicted_y1_y0 = boot_second_stage,
        treatment = treatment,
        mode = mode,
  ...
  structure(
    ...
    class = c("u_learner", "causal_learner")
  )
}
